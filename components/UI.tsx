  ...
  <div className={`bg-white p-5 rounded-xl border-l-4 shadow-sm ${color} flex flex-col justify-between h-full border border-slate-100 transition-transform hover:scale-[1.02]`}>
    <div>
      <div className="flex justify-between items-start mb-3">
        ...
        <span className="text-xs font-black text-slate-900 bg-slate-100 px-2 py-0.5 rounded uppercase tracking-tighter">{Number(percent).toFixed(2)}%</span>
      </div>
...
export const ProgressBar: React.FC<ProgressBarProps> = ({ label, value, relativeWidth, amount, colorClass }) => (
  <div className="mb-6 break-inside-avoid">
    <div className="flex justify-between items-center text-[10px] mb-2 font-bold tracking-wider uppercase">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-slate-300 text-[9px]">/ {Number(value).toFixed(2)}%</span>
      </div>
    </div>
    <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
      <div className={`h-full rounded-full ${colorClass} transition-all duration-700 ease-out`} style={{ width: `${relativeWidth ?? value}%` }}></div>
    </div>
  </div>
);
